...
export default function OnboardingPage() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/onboarding`, {
        headers: {
  ...
  const updateSelectedStep = (updater: (step: OnboardingStep) => OnboardingStep) => {
  ...
  const updateContentMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : "Failed to update step content");
    },
  });
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="container mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-gray-900">Onboarding Process Management</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-5">
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
              <div className="bg-gray-50 border-b border-gray-200 px-4 py-3">
                <p className="text-sm font-semibold text-gray-700">Onboarding Steps</p>
              </div>
            ...
            <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
              <div className="bg-gray-50 border-b border-gray-200 px-4 py-3">
                <p className="text-sm font-semibold text-gray-700">
                  {selectedStep ? `Step ${selectedStep.stepNumber}: ${selectedStep.name}` : "Select a step"}
                </p>
              </div>

              {!selectedStep ? (
                <div className="p-6 text-sm text-gray-500">No step selected.</div>
              ) : (
                <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
                  <div className="pt-4">
                    <p className="text-sm font-semibold text-gray-800 mb-3">Content</p>

                    <div className="mb-4">
                      <label className="block text-xs font-medium text-gray-700 mb-1">Step Key</label>
                      <input
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateSelectedStep((step) => ({
                              ...step,
                              content: { ...step.content, title: e.target.value },
                            }))
                          }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateSelectedStep((step) => ({
                              ...step,
                              content: { ...step.content, subtitle: e.target.value },
                            }))
                          }
                          className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-blue-400"
                        />
                    ...
                    <div className="mb-4">
                      ...
                      {selectedStep.content.fields.length === 0 && (
                        <p className="text-xs text-gray-400">No fields in this step.</p>
                      )}

                      <div className="space-y-3">
                        {selectedStep.content.fields.map((field, idx) => (
                          <div key={`${field.key}-${idx}`} className="border border-gray-200 rounded-lg p-3">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateSelectedStep((step) => {
                                      const nextFields = [...step.content.fields];
                                      nextFields[idx] = { ...nextFields[idx], label: e.target.value };
                                      return { ...step, content: { ...step.content, fields: nextFields } };
                                    })
                                  }
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
                              ...
                              <div>
                                <label className="block text-xs text-gray-600 mb-1">Helper Text</label>
                                <input
                                  value={field.helperText}
                                  onChange={(e) =>
                                    updateSelectedStep((step) => {
                                      const nextFields = [...step.content.fields];
                                      nextFields[idx] = { ...nextFields[idx], helperText: e.target.value };
                                      return { ...step, content: { ...step.content, fields: nextFields } };
                                    })
                                  }
                                  className="w-full border border-gray-200 rounded-md px-2.5 py-2 text-sm focus:outline-none focus:border-blue-400"
                              ...
                              <label className="flex items-center gap-1.5 text-xs text-gray-700">
                                <input
                                  ...
                                  onChange={(e) =>
                                    updateSelectedStep((step) => {
                                      const nextFields = [...step.content.fields];
                                      nextFields[idx] = { ...nextFields[idx], enabled: e.target.checked };
                                      return { ...step, content: { ...step.content, fields: nextFields } };
                                    })
                                  }
                                />
                    ...
                    <div className="mb-4">
                      ...
                      {selectedStep.content.roles.length === 0 && (
                        <p className="text-xs text-gray-400">No roles in this step.</p>
                      )}

                      <div className="space-y-3">
                        {selectedStep.content.roles.map((role, idx) => (
                          <div key={`${role.key}-${idx}`} className="border border-gray-200 rounded-lg p-3">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <input
                                value={role.key}
                                disabled
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  updateSelectedStep((step) => {
                                    ...
                                    nextRoles[idx] = { ...nextRoles[idx], label: e.target.value };
                                    return { ...step, content: { ...step.content, roles: nextRoles } };
                                  })
                                }
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                updateSelectedStep((step) => {
                                  const nextRoles = [...step.content.roles];
                                  nextRoles[idx] = { ...nextRoles[idx], description: e.target.value };
                                  return { ...step, content: { ...step.content, roles: nextRoles } };
                                })
                              }
                    ...
                    <div>
                      <p className="text-xs font-semibold text-gray-700 mb-2">
                        Entity Options ({selectedStep.content.entityOptions.length})
                      </p>

                      {selectedStep.content.entityOptions.length === 0 && (
                        <p className="text-xs text-gray-400">No entity options in this step.</p>
                      )}

                      <div className="space-y-3">
                        {selectedStep.content.entityOptions.map((entity, idx) => (
                          <div key={`${entity.key}-${idx}`} className="border border-gray-200 rounded-lg p-3">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              <input
                                value={entity.key}
                                disabled
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  updateSelectedStep((step) => {
                                    const nextEntities = [...step.content.entityOptions];
                                    nextEntities[idx] = { ...nextEntities[idx], label: e.target.value };
                                    return {
                                      ...step,
                                      content: { ...step.content, entityOptions: nextEntities },
                                    };
                                  })
                                }
