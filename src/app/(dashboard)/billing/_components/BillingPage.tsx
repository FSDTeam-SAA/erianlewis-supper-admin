...
function BillingPage() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/billing?${params.toString()}`,
        { headers: { Authorization: `Bearer ${token}` } },
      );
      const json = await res.json();
      if (!res.ok || !json?.status) {
  ...
  const planOptions = useMemo(() => {
    ...
    return items.map((plan: any) => ({
      id: plan._id,
      label: plan.title || plan.name || "Unknown Plan",
    }));
  }, [plansResponse]);
              ...
              billingRows.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-sm text-[#1C1C1C] capitalize">
