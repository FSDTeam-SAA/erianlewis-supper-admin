...
type ExpandedSectionType = 'contact' | 'social' | 'kb' | 'privacy' | 'license' | null;
...
const SOCIALS: Omit<SocialItem, 'enabled' | 'url'>[] = [
  { id: 'fb', name: 'Facebook', placeholder: 'https://facebook.com/yourusername' },
  { id: 'ig', name: 'Instagram', placeholder: 'https://instagram.com/yourusername' },
  { id: 'tt', name: 'TikTok', placeholder: 'https://tiktok.com/yourusername' },
  { id: 'li', name: 'LinkedIn', placeholder: 'https://linkedin.com/yourusername' },
  { id: 'yt', name: 'YouTube', placeholder: 'https://youtube.com/yourusername' },
];
...
const StatusBadge = ({ configured }: { configured: boolean }) => (
  <p className={`text-xs mb-4 ${configured ? 'text-green-600' : 'text-gray-400'}`}>
    {configured ? 'Configured' : 'Not Configured'}
  </p>
);

const RedButton = ({ onClick, children }: { onClick: () => void; children: React.ReactNode }) => (
  <button
    onClick={onClick}
...
const DocTabSwitcher = ({
  active, onChange, textarea, pdfInput,
}: {
  active: DocTabType;
...
export default function LegalDocuments() {
  ...
  const buildSettingsFormData = () => {
    ...
    formData.append('licensingInformationText', payload.licensingInformationText);
  ...
  const updatemutation = useMutation({
    mutationFn: async ({ body, isFormData }: { body: SettingsUpdatePayload | FormData; isFormData: boolean }) => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/settings`, {
        method: 'PUT',
        headers: {
  ...
  const saveAllSettings = async (onSuccess: () => void) => {
    try {
      ...
      await updatemutation.mutateAsync({
        body: shouldSendFormData ? buildSettingsFormData() : buildSettingsPayload(),
  ...
  const handleTermsImageUpload = () => {
    ...
    input.onchange = () => {
      ...
      reader.onload = () => {
        ...
        if (!imageSrc) return;
        setTosText((prev) => `${prev}<p><img src="${imageSrc}" alt="uploaded image" /></p>`);
      };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
              ...
              {expandedSection !== 'contact' ? (
                <RedButton onClick={() => setExpandedSection('contact')}>Configure</RedButton>
              ) : (
                <>
                  ...
                  <p className="text-sm font-medium text-gray-700 mb-2 mt-3">Email Addresses</p>
                  {emails.map((em, i) => (
                    <div key={i} className="flex gap-2 mb-2">
                      <input
                        value={em}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-gray-900">Manage Social</span>
              </div>
              <StatusBadge configured={socialConfigured} />
              {expandedSection !== 'social' ? (
                <RedButton onClick={() => setExpandedSection('social')}>Configure</RedButton>
              ) : (
                <>
                  {socials.map((s) => (
                    <div key={s.id} className="mb-4">
                      <div className="flex items-center justify-between mb-1.5">
                        ...
                        <Toggle on={s.enabled} onToggle={() => toggleSocial(s.id)} />
                      </div>
                      <input
                        value={s.url}
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm lg:col-span-2">
              ...
              {expandedSection !== 'kb' ? (
                <RedButton onClick={() => setExpandedSection('kb')}>Configure</RedButton>
              ) : (
                <>
                  <input
                    value={kbUrl}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
                ...
                pdfInput={
                  <label className="block cursor-pointer border-2 border-dashed border-gray-300 hover:border-red-400 rounded-lg p-6 text-center mb-3 transition-colors">
                    <input
                      ...
                      className="hidden"
                      onChange={(e) => setTosPdfFile(e.target.files?.[0] || null)}
                    />
                    <p className="text-sm text-gray-600">
                      {tosPdfFile ? tosPdfFile.name : 'Click to select Terms of Service PDF'}
                    </p>
                    <p className="text-xs text-gray-400 mt-1">Only .pdf file supported</p>
                  </label>
                }
              />
              <div className="flex gap-2">
                ...
                <>
                  <div className="bg-gray-50 rounded-lg border border-gray-100 p-3 mb-3 min-h-[80px] max-h-[140px] overflow-hidden">
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: ppText || '<p>No preview available</p>' }}
                    />
                  </div>
                  <RedButton onClick={() => setExpandedSection('privacy')}>Configure</RedButton>
                </>
              ) : (
                <>
                  <DocTabSwitcher
                    ...
                    pdfInput={
                      <label className="block cursor-pointer border-2 border-dashed border-gray-300 hover:border-red-400 rounded-lg p-6 text-center mb-3 transition-colors">
                        ...
                        />
                        <p className="text-sm text-gray-600">
                          {ppPdfFile ? ppPdfFile.name : 'Click to select Privacy Policy PDF'}
                        </p>
                        <p className="text-xs text-gray-400 mt-1">Only .pdf file supported</p>
                      </label>
                    }
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm lg:col-span-2">
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-gray-900">Licensing Information</span>
              </div>
              <StatusBadge configured={licConfigured} />
              {expandedSection !== 'license' ? (
                <RedButton onClick={() => setExpandedSection('license')}>Configure</RedButton>
              ) : (
                <>
                  <DocTabSwitcher
                    active={licTab}
                    ...
                    pdfInput={
                      <label className="block cursor-pointer border-2 border-dashed border-gray-300 hover:border-red-400 rounded-lg p-6 text-center mb-3 transition-colors">
                        <input
                          ...
                          accept="application/pdf"
                          className="hidden"
                          onChange={(e) => setLicPdfFile(e.target.files?.[0] || null)}
                        />
                        <p className="text-sm text-gray-600">
                          {licPdfFile ? licPdfFile.name : 'Click to select Licensing Information PDF'}
                        </p>
                        <p className="text-xs text-gray-400 mt-1">Only .pdf file supported</p>
                      </label>
                    }
